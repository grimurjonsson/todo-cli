...
mod tests {
    ...
    fn save_to_test_db(conn: &Connection, list: &TodoList) {
        ...
        conn.execute("DELETE FROM todos WHERE date = ?1", [&date_str]).unwrap();
    ...
    fn load_from_test_db(conn: &Connection, date: NaiveDate) -> Vec<TodoItem> {
        ...
        let mut result = Vec::new();
        for item in items {
            let (id_str, content, state_str, indent_level, parent_id_str, due_date_str, description) = item.unwrap();
    ...
    fn test_due_date_and_description_persisted() {
        ...
        assert_eq!(loaded[0].description, Some("This is a description".to_string()));
    }
}
